...
function WalkSimulator.new(controller)
    ...
    self.char_collision_box = self.controller:character().prototype
                                  .collision_box
    self.entity_cache = {}
    self.cache_box = nil
    self.walk_calls = 0
...
function WalkSimulator:check_prediction()
    ...
    if self.controller:tick() ~= self.previous_tick + 1 then
        log(
            "Can't check prediction not from the previous step. previous_tick = ",
            self.previous_tick, " current tick = ", self.controller:tick())
        return false
    end
    if player_pos ~= self.predicted_pos then
        if self.old_pos ~= self.predicted_pos or
           player_pos == self.predicted_no_collision then
