...
function Pathfinder:next_step()
  ...
  if self.closest_node_distance ~= nil and closest_node:remaining_steps() >
    ...
    --     "(cache: ", self.cache_hits, "/", self.cache_misses, ")")
    if #self.reversed_path > 0 then return table.remove(self.reversed_path) end
...
function Pathfinder:debug()
  ...
  log("Direct distance:", self.goals_pset:min_l2_dist(start) - self.goal_radius)
